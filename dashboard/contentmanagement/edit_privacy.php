...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Check if privacy content is empty
        if (empty($privacy_content)) {
            throw new Exception("Privacy Policy content cannot be empty.");
        }

        // First, check if privacy policy already exists
        $stmt = $pdo->prepare("SELECT id FROM privacy_policy LIMIT 1");
        $stmt->execute();
        $existing = $stmt->fetch();

        if ($existing) {
            // Update existing privacy policy
            $stmt = $pdo->prepare("UPDATE privacy_policy SET 
                content = :content,
                updated_at = NOW()
                WHERE id = :id");
            $stmt->execute([
            ...
            // Insert new privacy policy
            $stmt = $pdo->prepare("INSERT INTO privacy_policy (content, created_at, updated_at) 
                VALUES (:content, NOW(), NOW())");
            $stmt->execute(['content' => $privacy_content]);
        }
        ...
        $log_stmt = $pdo->prepare("INSERT INTO admin_logs (admin_id, action, action_detail, created_at) 
            VALUES (:admin_id, 'update_privacy', 'Privacy Policy updated', NOW())");
...
try {
    $stmt = $pdo->prepare("SELECT * FROM privacy_policy ORDER BY updated_at DESC LIMIT 1");
    $stmt->execute();
    ...
    <div class="min-h-screen">
        ...
        <header class="bg-white shadow-sm">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
                <div class="flex justify-between items-center">
                    <h1 class="text-2xl font-semibold text-gray-900">Edit Privacy Policy</h1>
        ...
        <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="bg-white shadow rounded-lg overflow-hidden">
                <form method="POST" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" x-data="{ privacyAccepted: false }" @submit.prevent="privacyAccepted ? $el.submit() : $event.preventDefault()">
                    <div class="p-6">
                        <div class="mb-6">
                            ...
                            <textarea 
                                ...
                                class="hidden"
                            ><?php echo htmlspecialchars($privacy['content'] ?? ''); ?></textarea>
                            <p class="mt-2 text-sm text-gray-500">
                                Use the editor above to format your privacy policy. You can add headings, lists, and other formatting as needed.
                            </p>
                        </div>
                        ...
                        <div class="flex items-start mb-6">
                            ...
                            <div class="ml-3 text-sm">
                                <label for="privacy_acceptance" class="font-medium text-gray-700">
                                    I confirm that these changes comply with all applicable privacy laws and regulations
                                </label>
                                <p class="text-red-600 mt-1" x-show="!privacyAccepted">
                    ...
                    <div class="px-6 py-4 bg-gray-50 text-right">
                        <button
                            type="submit"
                            :class="{'opacity-50 cursor-not-allowed': !privacyAccepted, 'hover:bg-primary-700': privacyAccepted}"
            ...
            <?php if (isset($privacy['updated_at'])): ?>
            <div class="mt-4 text-sm text-gray-500">
                Last updated: <?php echo date('F j, Y, g:i a', strtotime($privacy['updated_at'])); ?>
            </div>
            <?php endif; ?>
        </main>
    </div>
